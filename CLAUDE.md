...
| `js/main.js` | `reset()`, `update()` call order, game loop, scenes (`title`/`game`/`over`/`win`), per-level best-times tables (`allTimes` in localStorage `cq-times-v2`; old `cq-times` migrates to level 1), level progression (`unlockedCount` in localStorage `cq-unlocked` — the quests are beaten IN ORDER, a clean win opens the next one for good, earlier ones stay replayable; `isUnlocked(i)` / `startSelected()` gate the title screen, 👑 King Mode bypasses it so admin can skip levels, and cheated runs unlock nothing), and the transferred meat inventory (`carriedMeat` in localStorage `cq-meat` — seeds `res.meat` each level, `update()` banks changes back) |
